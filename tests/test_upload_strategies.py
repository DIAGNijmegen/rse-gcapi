...
def test_file_civ_clean(source, context, interface_kind):
    proto_civ = FileCIVCreateStrategy(
        source=source,
...
def test_image_civ_clean(source, context):
    proto_civ = ImageCIVCreateStrategy(
        source=source,
...
def test_value_civ_clean(source, context):
    proto_civ = ValueCIVCreateStrategy(
        source=source,
...
@sync_generator_test
def test_job_creation_clean(algorithm, inputs, context):
    job = JobInputsCreateStrategy(
        algorithm=algorithm,
        inputs=inputs,
        client_api=MagicMock(),
    )
    with context:
        yield from job.prepare()
